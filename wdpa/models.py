...
class WdpaAbstract(models.Model):
    ...
    wdpaid = (
        models.FloatField()
    )  # this comes in as float, should we convert to int? No records have 0.X decimal values
    ...
    def get_point_within(self):
        ...
        if self.point_within is None:
            ...
            me = (
                self.__class__.objects.centroid(field_name="geom")
                .point_on_surface(field_name="geom")
                .only("geom")
                .get(pk=self.pk)
            )
            # if the centroid intersects the polygon, use it, otherwise return the point_on_surface
            centroid_inside = self.__class__.objects.filter(
                pk=self.pk, geom__intersects=me.centroid
            ).count()
    ...
    def get_bbox(self):
        ...
        if self.bbox is None:
            # Make PostGIS calculate this for us
            me = (
                self.__class__.objects.envelope(field_name="geom")
                .only("id")
                .get(pk=self.pk)
            )
    ...
    def export_field_names(self):
        return wdpa_export_fields

    @property
    def export_dict(self):
        export_fields = wdpa_export_fields[:]  # copy list for local manipulation
        if "categories" in wdpa_export_fields:
            export_fields.remove("categories")
        if "geom" in export_fields:
            export_fields.remove("geom")
        export_dict = (
            self.__class__.objects.filter(pk=self.pk).values(*export_fields).first()
        )
