...
function LeafClusterer(map, opt_markers, opt_opts) {
	...
	function addLeftMarkers_() {
		...
		var leftMarkers = [];
		var i = 0, len = leftMarkers_.length.length;
		for (i = 0; i < len; i++) {
	...
	this.getTotalMarkers = function () {
		var i = 0, len = clusters_.length, result = 0;
		for (i = 0; i < len; i++) {
	...
	this.resetViewport = function () {
		...
		for (i = 0; i < len; i++) {
			...
			var oldZoom = null;
			if (cluster) {
				oldZoom = cluster.getCurrentZoom();
			}
			if (oldZoom === null) {
				continue;
			}
...
function Cluster(leafClusterer) {
	...
	var hidden_ = leafClusterer.isHidden();
	...
	this.isInBounds = function (bounds) {
		...
		var inViewport = true;
		var gridSize = leafClusterer.getGridSize();
	...
	this.redraw_ = function (isForce) {
		...
		if (zoom_ >= mz || this.getTotalMarkers() === 1) {
			...
			// have only one marker, the marker(s) in cluster will be shown on map.
			for (i = 0; i < len; i++) {
				map_.addLayer(markers_[i].marker);
				markers_[i].isAdded = true;
				if (hidden_) {
					markers_[i].marker._icon.style.display = 'none';
					markers_[i].marker._shadow.style.display = 'none';
				}
			}
			...
			if (clusterMarker_ === null) {
				clusterMarker_ = new ClusterMarker_(center_, this.getTotalMarkers(), leafClusterer_.getStyles_(), leafClusterer_.getGridSize() / 2);
	...
	/**
	 * Show this cluster and all the markers from this cluster.
	 */
	this.show = function () {
		if (clusterMarker_ !== null) {
			clusterMarker_.show();
		...
		for (i = 0; i < len; i++) {
			if (markers_[i].isAdded) {
				markers_[i].marker._icon.style.display = '';
				markers_[i].marker._shadow.style.display = '';
			}
		}
		hidden_ = false;
	};
	...
	/**
	 * Hide this cluster and all the markers from this cluster.
	 */
	this.hide = function () {
		if (clusterMarker_ !== null) {
			clusterMarker_.hide();
