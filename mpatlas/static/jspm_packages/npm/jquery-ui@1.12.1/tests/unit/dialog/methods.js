...
"format cjs";
(function(process) {
  define(["qunit", "jquery", "ui/widgets/dialog"], function(QUnit, $) {
    QUnit.module("dialog: methods", {afterEach: function() {
    ...
    QUnit.test("destroy", function(assert) {
      ...
      element = $("#dialog1").dialog({modal: true}), element2 = $("#dialog2").dialog({modal: true});
    ...
    QUnit.test("moveToTop", function(assert) {
      ...
      var dialog1,
          dialog2,
          focusOn = "dialog1";
      dialog1 = $("#dialog1").dialog({focus: function() {
          assert.equal(focusOn, "dialog1");
        }});
      focusOn = "dialog2";
      dialog2 = $("#dialog2").dialog({focus: function() {
    ...
    QUnit.test("moveToTop: content scroll stays intact", function(assert) {
      ...
      var otherDialog = $("#dialog1").dialog(),
          scrollDialog = $("#form-dialog").dialog({height: 200});
    ...
    QUnit.test("Ensure form elements don't reset when opening a dialog", function(assert) {
      assert.expect(2);
      var d1 = $("<form><input type='radio' name='radio' id='a' value='a' checked='checked'></input>" + "<input type='radio' name='radio' id='b' value='b'>b</input></form>").appendTo("body").dialog({autoOpen: false});
    ...
    QUnit.test("#8958: dialog can be opened while opening", function(assert) {
      ...
      $("body").trigger("focus");
      $("#favorite-animal").on("focus", function() {
        element.dialog("open");
      }).on("blur", function() {
        element.dialog("open");
      }).trigger("focus");
    ...
    QUnit.test("#5531: dialog width should be at least minWidth on creation", function(assert) {
      ...
      element.remove();
      element = $("<div></div>").dialog({minWidth: 300});
