...
function writeMessage(ctx, options) {
  ...
  if (!options.noRead) {
    ...
    for (var i = 0; i < fields.length; i++) {
      ...
      code += '    ' + (i ? 'else if' : 'if') + ' (tag === ' + field.tag + ') ' + (field.repeated && !packed ? 'obj.' + field.name + '.push(' + readCode + ')' : field.repeated && packed ? readCode : 'obj.' + field.name + ' = ' + readCode);
      if (field.oneof) {
  ...
  if (!options.noWrite) {
    ...
    for (i = 0; i < fields.length; i++) {
      field = fields[i];
      var writeCode = field.repeated && !isPacked(field) ? compileRepeatedWrite(ctx, field, numRepeated++) : compileFieldWrite(ctx, field, field.name);
...
function getType(ctx, field) {
  ...
  return path.reduce(function(ctx, name) {
    return ctx && ctx[name];
  }, ctx);
}
function compileFieldRead(ctx, field) {
  ...
  if (type) {
    if (type._proto.fields)
      return type._name + '.read(pbf, pbf.readVarint() + pbf.pos)';
    if (!isEnum(type))
...
function compileFieldWrite(ctx, field, name) {
  ...
  if (type) {
    if (type._proto.fields)
      return prefix + 'Message(' + field.tag + ', ' + type._name + '.write, obj.' + name + ')';
    if (type._proto.values)
...
function compileRepeatedWrite(ctx, field, numRepeated) {
  return 'for (' + (numRepeated ? '' : 'var ') + 'i = 0; i < obj.' + field.name + '.length; i++) ' + compileFieldWrite(ctx, field, field.name + '[i]');
}
