    ...
    def zones_display(self, obj):
        return mark_safe(
            ", ".join(
                [
                    '<a href="%s">%s</a>'
                    % (reverse("admin:mpa_mpa_change", args=(zone.mpa_id,)), zone.name)
...
class MpaAdmin(VersionAdmin, admin.GISModelAdmin):
    ...
    fieldsets = [
        ...
        (
            "Designation Type",
            {"fields": ["designation_type", "int_criteria", "iucn_category"]},
        ),
        ("Categories/Tags", {"fields": ["categories"]}),
        (
            "Is this a Marine Protected Area or a different marine managed area?",
            {"fields": ["is_mpa"]},
        ),
        (
            "Status",
        ...
        (
            "Region, Jurisdiction & Management",
            {
                "fields": [
                    "sovereign",
                    "country",
        ...
        (
            ...
            {"fields": ["fishing", "fishing_info", "fishing_citation"]},
        ),
        (
            "Access Restrictions",
            {"fields": ["access", "access_info", "access_citation"]},
        ),
        (
            "Protection Focus",
            {
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        remaining_fields = get_fields_missing_from_fieldsets(
            fieldsets, [f for f in fields_for_model(Mpa)]
        )
        if remaining_fields:
    ...
    def colored_verification_state(self, obj):
        if obj.verification_state == "Rejected as MPA":
            ...
            color_code = "000"
        return mark_safe(
            '<span style="color: #%s;">%s</span>' % (color_code, obj.verification_state)
        )
...
# admin.site.register(MpaCandidate, admin.GISModelAdmin)
