...
mpas_noproposed_nogeom = mpas_norejects.exclude(status="Proposed").defer(
    *Mpa.get_geom_fields()
)
mpas_proposed_nogeom = mpas_norejects.filter(status="Proposed").defer(
    *Mpa.get_geom_fields()
)
    ...
    if request.POST:
        ...
        if editform.is_valid():
            ...
            try:
                reversion.add_meta(
                    VersionMetadata,
                    comment=editform.cleaned_data.get("edit_comment"),
                    reference=editform.cleaned_data.get("edit_reference"),
                )
            except:
                pass
    ...
    def get_queryset(self):
        ...
        try:
            ...
            if q:
                ...
                vector = (
                    SearchVector("name", config=lang)
                    + SearchVector("designation", config=lang)
                    + SearchVector("designation_eng", config=lang)
                )
                vector_simple = (
                    SearchVector("name", config=simple_unaccent)
                    + SearchVector("designation", config=simple_unaccent)
                    + SearchVector("designation_eng", config=simple_unaccent)
                )
                ...
                if shortterms:
                    query_simple = PrefixedPhraseQuery(
                        " ".join(shortterms), config=simple_unaccent, all_partial=True
                    )
                    mysearch = mysearch & (Q(search_simple=query_simple))
                qs = qs.annotate(search_simple=vector_simple, search=vector).filter(
                    mysearch | Q(search=query)
                )
            qs = qs.order_by("name")
...
class MpaJsonListView(MpaListView):
    def render_to_response(self, context, **kwargs):
        # Reformat to use JsonResponse and json_dumps methods to format valid
        # json document that can handle line breaks, etc.
        return super(MpaJsonListView, self).render_to_response(
...
def get_mpa_geom_wkt(request, pk, simplified=True, webmercator=False):
    ...
    return HttpResponse(
        getattr(mpa, geomfield).wkt, content_type="text/plain; charset=utf-8"
    )
...
def get_mpa_geom_json(request, pk, simplified=True, webmercator=False):
    ...
    if webmercator:
        geomfield = "geom_smerc"
    mpaq = (
        Mpa.objects.annotate(geojson=AsGeoJSON(geomfield))
...
def lookup_point(request):
    ...
    try:
        ...
        if method == "webmercator":
            ...
            mpa_list = mpas_valid.filter(geog__intersects=searchbuffer).defer(
                *Mpa.get_geom_fields()
            )
            search = searchbuffer
            ...
            mpa_list = mpas_valid.filter(geog__intersects=searchbuffer.envelope).defer(
                *Mpa.get_geom_fields()
            ...
            mpa_list = mpas_valid.filter(geog__intersects=point).defer(
                *Mpa.get_geom_fields()
            )
            search = point
        candidate_radius = (
            radius * 2.2
        )  # We're using big icons on a point, this let's us catch it better
        mpa_point_list = mpas_norejects_nogeom.filter(
            is_point=True,
            point_geog__dwithin=(origpoint, Distance(km=candidate_radius)),
        )
        ...
        return render(
            ...
            "mpa/mpalookup.json",
            {
                ...
                "mpa_list": mpa_list
                | mpa_point_list,  # this joins the query sets with an OR
            },
            content_type="application/json; charset=utf-8",
        )
