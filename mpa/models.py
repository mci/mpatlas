...
MPA_TYPE_CHOICES = (
    ...
    (
        "(Non-MPA) Terrestrial Area without Marine Component",
        "(Non-MPA) Terrestrial Area without Marine Component",
    ),
)
...
PROTECTION_LEVEL_RBCS_CHOICES = [
    (i, "%d: %s" % (i, j)) for (i, j) in rbcs1_levels.items()
]
...
class Site(models.Model):
    ...
    wdpa_id = models.IntegerField(
        "WDPA ID", null=True, blank=True, help_text="WDPA ID code."
    )
    usmpa_id = models.CharField(
        "US MPA ID",
        max_length=50,
        null=True,
        blank=True,
        help_text="US NOAA MPA Center ID. You probably should not be changing this.",
    )
    other_ids = models.CharField(
        "Other reference id codes",
        max_length=1000,
        null=True,
        blank=True,
        help_text="ID codes used by other groups to identify this area, e.g., TNC Caribbean or Coral Triangle Atlas ids.",
    )
    name = models.CharField(
        "Name",
        max_length=254,
        blank=True,
        help_text="Protected area site name not including designation title",
    )
    orig_name = models.CharField(
        ...
        blank=True,
        default="",
        help_text="Protected area original site name not including designation title",
    )
    short_name = models.CharField(
        max_length=254, blank=True, help_text="Nickname if any"
    )  # name + designation with abbreviations
    slug = models.CharField(
        "URL Slug", max_length=254, blank=True, help_text="URL slug string"
    )

    designation = models.CharField("Designation", max_length=254, null=True, blank=True)
    designation_eng = models.CharField(
        "English Designation", max_length=254, null=True, blank=True
    )
    ...
    categories = TaggableManager(
        ...
        help_text='You can assign this site to one or more categories by providing a comma-separated list of tags enclosed in quotes (e.g., [ "Shark Sanctuary", "World Heritage Site" ]',
        blank=True,
    )
    glores_status = models.CharField(
        "GLORES Status", max_length=100, blank=True, choices=GLORES_CHOICES, default=""
    )
...
class Mpa(models.Model):
    ...
    wdpa_id = models.IntegerField(
        ...
        null=True,
        blank=True,
        help_text="WDPA ID code. You probably should not be changing this.",
    )
    wdpa_pid = models.CharField(
        "WDPA Parcel ID",
        max_length=52,
        null=True,
        blank=True,
        help_text="WDPA Parcel ID code in form wdpaid_X. You probably should not be changing this.",
    )
    usmpa_id = models.CharField(
        "US MPA ID",
        max_length=50,
    ...
    name = models.CharField(
        "Name",
        max_length=254,
        help_text="Protected area name not including designation title",
    )
    orig_name = models.CharField(
        "Original Name",
        max_length=254,
        blank=True,
        default="",
        help_text="Protected area original name not including designation title",
    )
    ...
    categories = TaggableManager(
        through=TaggedItem,
        verbose_name="Categories",
        help_text='You can assign this area to one or more categories by providing a comma-separated list of tags enclosed in quotes (e.g., [ "Shark Sanctuary", "World Heritage Site" ]',
        blank=True,
    )
    glores_status = models.CharField(
        "GLORES Status", max_length=100, blank=True, choices=GLORES_CHOICES, default=""
    ...
    sovereign = models.CharField(
        "Sovereign Country", max_length=50, null=True, blank=True
    )
    country = models.CharField("Country / Territory", max_length=50)
    sub_location = models.CharField(
        "Sub Location", max_length=100, null=True, blank=True
    )
    ...
    verification_reason = models.CharField(
        "Verification Reason", max_length=1000, null=True, blank=True
    )
    ...
    verify_wdpa = models.CharField(
        "Verification by WDPA",
        max_length=20,
        null=True,
        blank=True,
        default="Not Reported",
        choices=VERIFY_WDPA_CHOICES,
    )
    ...
    """
    wdpa_metadataid = models.IntegerField(
        "WDPA Source Metadata ID", null=True, blank=True, editable=False
    )

    # Modification History
    created_date = models.DateTimeField(
        "Creation Date",
        help_text="Date and time record created",
        auto_now_add=True,
        null=True,
    )
    modified_date = models.DateTimeField(
        "Modification Date",
        help_text="Date and time of last record save",
        auto_now=True,
        null=True,
    )
    ...
    designation_type = models.CharField(
        ...
        null=True,
        blank=True,
        choices=DESIG_TYPE_CHOICES,
    )
    iucn_category = models.CharField(
        "IUCN Category", max_length=20, null=True, blank=True, choices=IUCN_CAT_CHOICES
    )
    int_criteria = models.CharField(
        "International Criteria", max_length=100, null=True, blank=True
    )
    marine = models.IntegerField(
        "Marine (field from WDPA)", null=True, default=0, choices=MARINE_CHOICES
    )
    ...
    status_mpatlas = models.CharField(
        "Status (mpatlas value)",
        max_length=100,
        null=True,
        blank=True,
        choices=STATUS_CHOICES,
    ...
    status_year_mpatlas = models.IntegerField(
        "Status Year (mpatlas value)", null=True, blank=True
    )
    ...
    implemented = models.BooleanField(
        "Implemented?",
        help_text="MPA is designated and implemented with regulations enforced",
        blank=True,
        default=True,
    )
    implementation_date = models.DateField(
        "Implementation Date",
        help_text="Date regulations went into effect or will go into effect",
        null=True,
        blank=True,
    )

    # Area Estimates
    no_take = models.CharField(
        "No Take", max_length=100, choices=NO_TAKE_CHOICES, default="Not Reported"
    )
    ...
    no_take_wdpa = models.CharField(
        "No Take (wdpa value)",
        max_length=100,
        choices=NO_TAKE_CHOICES,
        default="Not Reported",
    )
    no_take_area_wdpa = models.FloatField(
        "No Take Area km² (wdpa value)", null=True, blank=True
    )
    no_take_mpatlas = models.CharField(
        "No Take (mpatlas value)",
        max_length=100,
        choices=NO_TAKE_CHOICES,
        default="Not Reported",
    )
    no_take_area_mpatlas = models.FloatField(
        "No Take Area km² (mpatlas value)", null=True, blank=True
    )
    calc_m_area_mpatlas = models.FloatField(
        "Calculated Marine Area km² (mpatlas value)", null=True, blank=True
    )
    calc_area_mpatlas = models.FloatField(
        "Calculated Area km² (mpatlas value)", null=True, blank=True
    )

    # Management details
    gov_type = models.CharField(
        "Governance Type", max_length=254, null=True, blank=True
    )  # = US gov_level
    ...
    mgmt_auth = models.CharField(
        "Management Authority", max_length=254, null=True, blank=True
    )
    mgmt_plan_type = models.CharField(
        "Management Plan Type", max_length=254, null=True, blank=True
    )
    mgmt_plan_ref = models.CharField(
        "Management Plan Reference", max_length=254, null=True, blank=True
    )

    # Contact
    contact = models.ForeignKey(
        "Contact",
        related_name="mpa_main_set",
    ...
    other_contacts = models.ManyToManyField(
        "Contact", verbose_name="Other Contacts", blank=True
    )

    # Conservation Effectiveness
    conservation_effectiveness = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=CONSERVATION_EFFECTIVENESS_CHOICES,
        default="Unknown",
    )

    # FISHING PROTECTION LEVEL
    fishing_protection_level = models.CharField(
        "Fishing Protection Level",
        max_length=100,
        default="unknown",
        blank=True,
        choices=FISHING_PROTECTION_LEVEL_CHOICES,
        editable=False,
    )
    fishing_protection_details = JSONField(
        "Fishing Protection Level Details", default=dict, editable=False
    )
    """EXAMPLE
        {
    ...
    # PROTECTION LEVEL
    protection_mpaguide_level = models.CharField(
        "Protection Level - MPA Guide",
        max_length=100,
        default="unknown",
        blank=True,
        choices=PROTECTION_LEVEL_MPAGUIDE_CHOICES,
        editable=False,
    )
    protection_mpaguide_details = JSONField(
        "Protection Level Details - MPA Guide", default=dict, editable=False
    )
    """EXAMPLE
        {
            'version': '1.0',
    ...
    protection_rbcs_level = models.IntegerField(
        "Protection Level - RBCS",
        default=99,
        blank=True,
        choices=PROTECTION_LEVEL_RBCS_CHOICES,
        editable=False,
    )
    ...
    protection_rbcs_level_name = models.CharField(
        "Protection Level Name - RBCS",
        max_length=100,
        default="unknown",
        blank=True,
        choices=PROTECTION_LEVEL_RBCS_NAME_CHOICES,
        editable=False,
    )
    protection_rbcs_details = JSONField(
        "Protection Level Details - RBCS", default=dict, editable=False
    )
    """EXAMPLE
        {
            'version': '1 | 2.1',
    ...
    fishing = models.CharField(
        ...
        null=True,
        blank=True,
        choices=FISHING_CHOICES,
        default="Unknown",
    )
    fishing_info = models.TextField(null=True, blank=True)
    fishing_citation = models.TextField(null=True, blank=True)
    access = models.CharField(
        max_length=254, null=True, blank=True, choices=ACCESS_CHOICES, default="Unknown"
    )
    access_info = models.TextField(null=True, blank=True)
    access_citation = models.TextField(null=True, blank=True)

    # Conservation Focus
    primary_conservation_focus = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=CONSERVATION_FOCUS_CHOICES,
        default="Unknown",
    )
    secondary_conservation_focus = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=CONSERVATION_FOCUS_CHOICES,
    ...
    tertiary_conservation_focus = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=CONSERVATION_FOCUS_CHOICES,
    ...
    # Protection Focus
    protection_focus = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=PROTECTION_FOCUS_CHOICES,
        default="Unknown",
    )
    ...
    constancy = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=CONSTANCY_CHOICES,
        default="Unknown",
    )
    constancy_citation = models.TextField(null=True, blank=True)
    permanence = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=PERMANENCE_CHOICES,
        default="Unknown",
    )
    permanence_citation = models.TextField(null=True, blank=True)

    # Notes
    wdpa_notes = models.CharField(
        "Area Notes (from WDPA)", max_length=250, null=True, blank=True, editable=False
    )
    ...
    geom_smerc = models.MultiPolygonField(
        srid=3857, null=True, blank=True, editable=False
    )
    geom = models.MultiPolygonField(srid=4326, null=True, blank=True, editable=False)
    geog = models.MultiPolygonField(
        srid=4326, geography=True, null=True, blank=True, editable=False
    )

    # Simplified polygon features
    simple_geom_smerc = models.MultiPolygonField(
        srid=3857, null=True, blank=True, editable=False
    )
    simple_geom = models.MultiPolygonField(
        srid=4326, null=True, blank=True, editable=False
    )
    simple_geog = models.MultiPolygonField(
        srid=4326, geography=True, null=True, blank=True, editable=False
    )

    # Point location, used when we don't have polygon boundaries
    point_geom_smerc = models.MultiPointField(
        srid=3857, null=True, blank=True, editable=False
    )
    point_geom = models.MultiPointField(
        srid=4326, null=True, blank=True, editable=False
    )
    point_geog = models.MultiPointField(
        srid=4326, geography=True, null=True, blank=True, editable=False
    )
    ...
    bbox_upperright = models.PointField(
        srid=4326, null=True, blank=True, editable=False
    )
        ...
        try:
            if self.is_point:
                self.point_within = self.point_geom[
                    0
                ]  # setting point with first feature in multipoint
            else:
                if not self.geom.valid:
                    return None
                me = (
                    self.__class__.objects.point_on_surface(field_name="geom")
                    .only("mpa_id")
                    .get(pk=self.pk)
                )
        ...
        try:
            if self.is_point:
                ...
                if not self.geom.valid:
                    return None
                extent = (
                    self.__class__.objects.only("mpa_id")
                    .filter(pk=self.pk)
                    .aggregate(Extent("geom"))
                    .get("geom__extent")
                )
                self.bbox_lowerleft = geos.Point(
                    extent[0], extent[1], srid=gdal.SpatialReference("WGS84").srid
                )
                self.bbox_upperright = geos.Point(
                    extent[2], extent[3], srid=gdal.SpatialReference("WGS84").srid
                )
            self.save()
    ...
    def clean_geom(self, resolution=1e-09, dry_run=False):
        ...
        if mpa.geom and mpa.geom_nodup.num_coords < mpa.geom.num_coords:
            ...
            logger.warning(
                ...
                mpa.geom.num_coords - mpa.geom_nodup.num_coords,
                mpa.mpa_id,
                mpa.name,
                mpa.designation,
                mpa.country,
    ...
    def make_simplified_geom(self, tolerance=0.005, preservetopology=False):
        st_simplify = (
            "ST_SimplifyPreserveTopology" if preservetopology else "ST_Simplify"
        )
        # Raw SQL update geometry fields, much faster than through django
        cursor = connection.cursor()
        cursor.execute(
            "UPDATE mpa_mpa SET simple_geom = ST_Multi(ST_MakeValid("
            + st_simplify
            + "(geom, %s))) WHERE mpa_id = %s",
            (tolerance, self.mpa_id),
        )
    ...
    def set_geom_from_geog(self):
        ...
        cursor = connection.cursor()
        cursor.execute(
            "UPDATE mpa_mpa SET geom = geog::geometry, geom_smerc = ST_TRANSFORM(geog::geometry, 3857) WHERE mpa_id = %s",
            [self.mpa_id],
        )
        cursor.execute(
            "UPDATE mpa_mpa SET point_geom = point_geog::geometry, point_geom_smerc = ST_TRANSFORM(point_geog::geometry, 3857) WHERE mpa_id = %s",
            [self.mpa_id],
        )
    ...
    def set_geog_from_geom(self):
        ...
        cursor = connection.cursor()
        cursor.execute(
            "UPDATE mpa_mpa SET geog = geom::geography, geom_smerc = ST_TRANSFORM(geom, 3857) WHERE mpa_id = %s",
            [self.mpa_id],
        )
        cursor.execute(
            "UPDATE mpa_mpa SET point_geog = point_geom::geography, point_geom_smerc = ST_TRANSFORM(point_geom, 3857) WHERE mpa_id = %s",
            [self.mpa_id],
        )
    ...
    def set_all_geom_from_geog(cls):
        ...
        cursor = connection.cursor()
        cursor.execute(
            "UPDATE mpa_mpa SET geom = geog::geometry, geom_smerc = ST_TRANSFORM(geog::geometry, 3857)"
        )
        cursor.execute(
            "UPDATE mpa_mpa SET point_geom = point_geog::geometry, point_geom_smerc = ST_TRANSFORM(point_geog::geometry, 3857)"
        )
...
class WikiArticle(models.Model):
    ...
    summary = RichTextField(
        "MPA Site Description from Wikipedia", null=True, blank=True
    )
...
class Contact(models.Model):
    ...
    logo = FilerImageField(
        null=True, blank=True, related_name="contact_logos", on_delete=models.SET_NULL
    )
...
class VersionMetadata(models.Model):
    revision = models.OneToOneField(
        Revision, on_delete=models.CASCADE
    )  # This is required
...
class CandidateInfo(models.Model):
    ...
    scope = models.CharField(
        ...
        null=True,
        blank=True,
        choices=CANDIDATE_SCOPE_CHOICES,
        default="Site",
    )
