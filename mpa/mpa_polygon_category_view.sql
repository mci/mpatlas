DROP VIEW public.mpa_category_view;

CREATE OR REPLACE VIEW public.mpa_category_view
AS SELECT m.mpa_id,
    m.geom,
    g.categories,
    m.wdpa_id,
   ...
   FROM mpa_mpa m,
    ( SELECT n.mpa_id,
          ...
          GROUP BY n.mpa_id
          ORDER BY n.mpa_id) g
  WHERE  m.mpa_id = g.mpa_id
  ORDER BY m.mpa_id DESC;

  -- Rules and Triggers
CREATE OR REPLACE RULE mpa_category_view_INSERT as
    ON INSERT TO mpa_category_view DO INSTEAD
    INSERT INTO mpa_mpa 
    (geom,
    wdpa_id,
    wdpa_pid,
...
CREATE OR REPLACE RULE mpa_category_view_UPDATE AS
    ON UPDATE TO mpa_category_view DO INSTEAD
    UPDATE mpa_mpa
    SET
        (mpa_id,
        geom,
...
CREATE OR REPLACE RULE mpa_category_view_DELETE as
    ON DELETE TO mpa_category_view DO INSTEAD
     DELETE FROM mpa_mpa
     WHERE mpa_id=OLD.mpa_id;

-- Permissions

ALTER TABLE public.mpa_category_view OWNER TO mpatlas;
GRANT ALL ON TABLE public.mpa_category_view TO mpatlas;


DROP VIEW public.mpa_polygon_category_view;
