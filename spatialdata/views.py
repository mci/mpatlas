...
class EezJsonListView(EezListView):
    def render_to_response(self, context, **kwargs):
        return super(EezJsonListView, self).render_to_response(
...
def get_geom_wkt(request, model, pk, simplified=True, webmercator=False):
    ...
    return HttpResponse(
        getattr(obj, geomfield).wkt, content_type="text/plain; charset=utf-8"
    )
...
def get_geom_json(request, model, pk, simplified=True, webmercator=False):
    ...
    if simplified:
        geomfield = "simple_" + geomfield
    obj = (
        model.objects.geojson(field_name=geomfield)
        .defer(*model.get_geom_fields())
        .get(pk=pk)
    )
...
def region_lookup_point(request, region):
    ...
    try:
        ...
        if method == "webmercator":
            ...
            ).defer(*region.get_geom_fields())
            search = point
        elif method == "webmercator_buffer":
            point.transform(3857)  # Proper Spherical Mercator srid
            searchbuffer = point.buffer(radius * 1000)  # convert km to m, create buffer
            region_list = region.objects.filter(geog__intersects=searchbuffer).defer(
                *region.get_geom_fields()
            )
            search = searchbuffer
            ...
            region_list = region.objects.filter(geog__intersects=point).defer(
                *region.get_geom_fields()
            )
            search = point
...
def country_lookup_point_old(request, region):
    ...
    try:
        ...
        if method == "webmercator":
            ...
            search = point
        search.transform(4326)
        country_list = Nation.objects.filter(
            pk__in=region_list.values_list("id", flat=True).distinct()
        )
        return render(
            request,
            "spatialdata/regionlookup.json",
