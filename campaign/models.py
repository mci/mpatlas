...
YEAR_CHOICES = [(None, None)] + [
    (r, r) for r in range(1990, datetime.date.today().year + 1)
]
...
class Campaign(models.Model):
    ...
    categories = TaggableManager(
        ...
        help_text="You can assign this area to one or more categories by providing a comma-separated list of tags (e.g., [ Shark Sanctuary, World Heritage Site ]",
        blank=True,
    )
    ...
    point_geom_smerc = models.PointField(
    ...
    start_year = models.IntegerField(
        _("Start Year"), choices=YEAR_CHOICES, null=True, blank=True
    )
    ...
    def process_geom_fields(self):
        ...
        cursor.execute(
            "UPDATE campaign_campaign SET geom_smerc = ST_TRANSFORM(geom, 3857) WHERE id = %s",
            [self.id],
        )
        cursor.execute(
            "UPDATE campaign_campaign SET point_geom_smerc = ST_TRANSFORM(point_geom, 3857) WHERE id = %s",
            [self.id],
        )
...
class Initiative(models.Model):
    ...
    # Summary Info
    logo = FilerImageField(
        verbose_name="Initiative Logo",
        related_name="initiative_logos",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
...
class Organization(models.Model):
    ...
    social_handles = models.CharField(
        "Social Media Handles", max_length=254, blank=True
    )
